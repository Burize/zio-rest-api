...
object UserRoutes:
  def apply() =
    Routes(
      ...
      Method.GET / "user" / "my" -> handler { (_: Request) => ZIO.serviceWith[String](
        name => Response.text(s"Welcome $name!")
      )},
      ...
      for {
        _ <- ZIO.logErrorCause(s"Failed to lookup user.", Cause.fail(error))
