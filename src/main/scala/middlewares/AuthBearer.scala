    ...
    request.header(Header.Authorization) match
      case Some(Header.Authorization.Bearer(token)) =>
        for
          ...
          claim          <- ZIO
                              .fromTry(jwtDecode(token.value.asString, config.session.jwt_secret_key))
                              .orElseFail(Response.unauthorized(INVALID_JWT_ERROR))
          userSession    <-
            ZIO.fromEither(claim.content.fromJson[UserSession]).orElseFail(Response.unauthorized(INVALID_JWT_ERROR))
